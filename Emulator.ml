...
module Binary =
  (struct
    ...
    (* TRANSLATION OF BANDS *)
    let rec (encode_on_one_band: Band.t -> Band.t) = fun band ->
      let alphabet = Alphabet.binary in
      let bandBinary = Band.make alphabet [] in
      let nbSymbols = band.alphabet.symbol_size_in_bits in
      encode_on_one_band band

    and (encode: Band.t list -> Band.t list) = (fun bands ->
